...
<meta property="og:title" content="${board.subject}"/>
<meta name="author" content="${board.nickname}"/>
<meta name="description"/>
<meta property="og:description" content='${fn:substring(board.content, fn:indexOf(board.content, "<p>") + 3, fn:indexOf(board.content, "</p>"))}'/>
<meta property="og:image" content='${fn:substring(board.content, fn:indexOf(board.content, "img src") + 9, fn:indexOf(board.content, "g\">") + 1)}'/>
<link rel="canonical" href="https://www.afreecaps.com/community/board/view/${board.id}?broadcasterId=${board.broadcaster_id}">
<meta property="og:url" content="https://www.afreecaps.com/community/board/view/${board.id}?broadcasterId=${board.broadcaster_id}"/>
...
<link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/resources/css/community/board/board_view.css">
...
$(document).ready(function() {
    var description = $("#content").html().replace(/<figure class=\"media\">.*?<\/figure>/g, '').replace(/<figure class=\"image\">.*?<\/figure>/g, '').trim();
    description = description.replace(/<p>/g, '').replace(/<\/p>/g, '');
    $("meta[name='description']").attr("content", description);
    $("meta[property='og:description']").attr("content", description);
    var ogImage = $("#content .image").html();
...
function moveBoardPage(page) {
	...
	if("${not empty param.searchType}" == "true") {
		location.href = "${pageContext.request.contextPath}/community/board?id=${broadcaster.id}&listType=" + listType + "&page=" + page + "&searchValue=${param.searchValue}&searchType=${param.searchType}";
	} else {
		location.href = "${pageContext.request.contextPath}/community/board?id=${broadcaster.id}&listType=" + listType + "&page=" + page;	
	}
}
// 게시글 추천/비추천
...
function deleteBoardWriteOk() {
	var form = $("<form></form>");
	form.attr("action", "${pageContext.request.contextPath}/community/board/deleteOk");
	form.attr("method", "post");
	form.append("<input type='hidden' id='id' name='id' value='${board.id}'/>");
	form.append("<input type='hidden' id='broadcasterId' name='broadcasterId' value='${broadcaster.id}'/>");
...
function searchBoard() {
	...
	location.href = "${pageContext.request.contextPath}/community/board?id=${broadcaster.id}&listType=" + listType + "&searchValue=" + searchValue + "&searchType=" + searchType;
}
        ...
        <div class="content-inner-box">
            <div class="container">
                ...
                <div class="board-function">
                    <button class="aps-button" onclick="location.href='${pageContext.request.contextPath}/community/board?id=${broadcaster.id}&page=${pagination.page}'"><i class="fas fa-list"></i> 목록</button>
                    <s:authorize access="isAuthenticated()">
                    	<c:if test="${board.user_id == sessionScope.id}">
                    		<button class="aps-button" onclick="location.href='${pageContext.request.contextPath}/community/board/modify/${board.id}?broadcasterId=${broadcaster.id}&page=${pagination.page}'"><i class="fas fa-edit"></i> 수정</button>
                    	</c:if>
                    </s:authorize>
					<s:authorize access="isAuthenticated()">
						<c:if test="${board.user_id == sessionScope.id}">
                ...
                <div class="board-neighbor">
                    <div class="neighbor-box">
                    	<c:if test="${board.next_id != 0 && not empty board.next_subject}">
                        	<div class="indicator"><i class="fas fa-chevron-up"></i> 다음글</div><div class="subject" onclick="location.href='${pageContext.request.contextPath}/community/board/view/${board.next_id}?broadcasterId=${broadcaster.id}&page=${pagination.page}'">${board.next_subject}</div>
                        </c:if>
                    </div>
                    <div class="neighbor-box">
                    	<c:if test="${board.prev_id != 0 && not empty board.prev_subject}">
                        	<div class="indicator"><i class="fas fa-chevron-down"></i> 이전글</div><div class="subject" onclick="location.href='${pageContext.request.contextPath}/community/board/view/${board.prev_id}?broadcasterId=${broadcaster.id}&page=${pagination.page}'">${board.prev_subject}</div>
                        </c:if>
                    </div>
                </div>
                <!-- 댓글 -->
                <div class="board-comment">
                    ...
                    <s:authorize access="isAuthenticated()">
	                    <div class="write-box">
	                        ...
	                        <div class="bottom">
	                            <button class="write-button" onclick="writeComment();">등록</button>
	                        </div>
	                    </div>
                    </s:authorize>
                    <s:authorize access="!isAuthenticated()">
                        	...
                        	<c:choose>
                        		<c:when test="${not empty boardList}">
                        			<c:forEach var="i" items="${boardList}" varStatus="index">
			                            <li class="bl-default" onclick="location.href='${pageContext.request.contextPath}/community/board/view/${i.id}?broadcasterId=${i.broadcaster_id}&listType=${listType}&page=${pagination.page}'">
			                                <div class="write-box">
 			                                	<div class="badge">
			                                		<c:choose>
...
<%-- 			                                	<c:choose>
                    ...
                    <div class="function-wrapper">
                        <button class="aps-button" onclick="location.href='${pageContext.request.contextPath}/community/board/write?id=${broadcaster.id}'"><i class="fas fa-pen"></i> 글쓰기</button>
                    </div>
