...
$(document).ready(function() {
    resize_find_password();
});
$(window).resize(resize_find_password);
function resize_find_password() {
...
function findPassword() {
	...
	$.ajax({
		url: "${pageContext.request.contextPath}/login/find/password",
		type: "POST",
		cache: false,
		data: {
			"name" : name,
		...
		success: function(data, status) {
			if(status == "success") {
				if(data == "Success") {
					modalToggle("#modal-type-1", "안내", "회원님의 이메일로 임시 비밀번호를 전송하였습니다.");
				} else {
    ...
    <div class="content-inner">
        <div class="content-inner-box">
            <div class="find-tab-box">
                <ul class="find-tab">
                    <li class="fl-default" onclick="location.href='${pageContext.request.contextPath}/login/find/username'">아이디 찾기</li>
                    <li class="fl-default active" onclick="location.href='${pageContext.request.contextPath}/login/find/password'">비밀번호 찾기</li>
                </ul>
            </div>
            <div class="find-content-box">
                ...
                <button class="aps-button" onclick="findPassword();">확인</button>
            </div>
        </div>
    </div>
</div>
